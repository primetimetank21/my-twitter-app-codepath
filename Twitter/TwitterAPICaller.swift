...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweetStr:String, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let url = "https://api.twitter.com/1.1/statuses/update.json"
        
        TwitterAPICaller.client?.post(url, parameters: ["status":tweetStr], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in success()
